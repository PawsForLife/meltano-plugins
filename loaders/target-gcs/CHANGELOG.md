...
- **target-gcs-extraction-patterns** — Base path pattern for extraction-pattern refactor. Details: [_archive/target-gcs-extraction-patterns/target-gcs-extraction-patterns.md](../../_archive/target-gcs-extraction-patterns/target-gcs-extraction-patterns.md) (when archived in Phase 6).
  ...
  - Task 07: Wire exports in `target_gcs.paths`: export BasePathPattern, SimplePath, DatedPath, PartitionedPath, PathType, DEFAULT_KEY_NAMING_CONVENTION, DEFAULT_KEY_NAMING_CONVENTION_HIVE via __all__; add import smoke test for sinks and paths public API; no behaviour change.
...
- **target-gcs-conftest-restructure** — Centralise test fixtures and helpers under conftest and test_helpers.
  ...
  - Task 05: Refactor `tests/unit/paths/test_simple.py` to use build_simple_path from conftest and key_from_open_call from test_helpers; remove local _build_simple_path and _key_from_open_call; use key_from_open_call(mock_open.call_args[0]) and key_from_open_call(c[0]) for call_args_list; keep per-test patch of target_gcs.paths.simple.smart_open.open. All test names and assertions unchanged.
  - Task 06: Refactor `tests/unit/paths/test_dated.py` to use build_dated_path from conftest and key_from_open_call from test_helpers; remove local build_dated_sink and _key_from_open_call; keep per-test patch of target_gcs.paths.dated.smart_open.open. All test names and assertions unchanged.
...
- **split-path-filename** — Details: [split-path-filename.md](../../_archive/split-path-filename/split-path-filename.md)
  ...
  - Task 08: Sinks config wiring: sink does not pass or use key_naming_convention; add test_key_shape_matches_constants; key shape from constants.
  - Task 09: Remove `get_partition_path_from_schema_and_record` and `helpers/partition_path.py`; `DEFAULT_PARTITION_DATE_FORMAT` single source in `constants.py`.
  - Task 10: Documentation: README (remove `key_naming_convention` from config table; update Hive section; add key format table; remove `chunk_index` from chunking); AI context (config schema, key tokens, path patterns, examples); CHANGELOG.
...
- **target-gcs-dedup-split-logic** — Unify partition date format constant: single source for `DEFAULT_PARTITION_DATE_FORMAT` in `partition_path.py`; sinks import it; removed local constant from sinks.py.
  ...
  - Add `_assert_field_required_and_non_null_type()` in `partition_schema.py`; refactor `validate_partition_fields_schema` and `validate_partition_date_field_schema` to use it (shared “in properties, required, non-null type” checks; custom `field_label`/reasons for partition_date_field). Helper not exported.
  - Task 09: Confirm Google-style docstrings for all new private methods and add brief comment in `_apply_key_prefix_and_normalize` (key normalization). Update `docs/AI_CONTEXT/AI_CONTEXT_target-gcs.md`: GCSSink private helpers, constant location in `partition_path`, partition_schema validators.
