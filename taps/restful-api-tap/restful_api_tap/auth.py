...
class ConfigurableOAuthAuthenticator(OAuthAuthenticator):
    ...
    def oauth_request_body(self) -> dict:
        ...
        on what config file (or stream-level config) keys have been set
        and the type of OAuth flow set by the grant_type.
        """
        # Resolve config source: tap-level from config file or stream-level config
        if self.config:  # config file (tap-level)
            my_config = self.config
        elif self._config:  # stream-level config
...
def select_authenticator(self) -> Any:
    ...
    the headers and params in the config file (or stream-level config).
    Note: Each auth method requires certain config file keys to be present; see README.md
    for each auth method's config file requirements.
...
def get_authenticator(self) -> Any:
    ...
    Uses config from the config file (tap-level) or stream-level config.
    If the authenticator already exists, use the cached Authenticator.
