"""Stream type classes for the tap (Singer extractor).

This module defines stream type classes used by restful-api-tap. Streams emit
SCHEMA and RECORD messages; terminology uses "stream" (not sink/target).
"""
...
class DynamicStream(RestApiStream):
    """Custom stream for the tap (not a sink).

    Defines how data for this stream is fetched and emitted as SCHEMA and
    RECORD messages.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            params: see tap.py
            headers: see tap.py
            primary_keys: primary keys (SDK); corresponds to key_properties in
                Singer SCHEMA messages. See tap.py.
            replication_key: property used for INCREMENTAL replication; last
                value stored in stream state (bookmarks). See tap.py.
            except_keys: see tap.py
            records_path: see tap.py
            next_page_token_path: see tap.py
            schema: JSON Schema for the stream (used in SCHEMA messages).
    ...
    def get_new_paginator(self):
        """Return the paginator used to page through API responses when syncing this stream.
    ...
    def _get_url_params_page_style(
        ...
        Args:
            context: optional - stream context (includes stream state/bookmarks for
    ...
    def parse_response(self, response: requests.Response) -> Iterable[dict]:
        """Parse the response and return an iterator of records (Singer RECORD payloads).
    ...
    def post_process(  # noqa: PLR6301
        ...
        Args:
            ...
            context: optional - stream context (stream state/bookmarks).

        Returns:
            A record that has been processed (flattened or unchanged per
            flatten_records), for emission as a RECORD message.
