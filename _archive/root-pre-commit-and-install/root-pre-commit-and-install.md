...
- **Failure behaviour:** Root install runs all plugin installs, prints a succeeded/failed summary, and exits non-zero if any plugin failed; wrapper script exits on first failing check. Pre-commit install runs after all plugin installs; root install installs pre-commit via pip if missing.

**Architecture:**

- **Root `install.sh`:** Resolve repo root, run `list_packages.py`, for each path run `(cd "$ROOT/$path" && ./install.sh)` and record success/failure (do not stop on first failure). Print a succeeded/failed summary and exit non-zero if any plugin failed. Then ensure pre-commit is available (install via pip if not), run `pre-commit install` at root.
