...
- **Task 06:** GCSSink refactored: pattern selection in `__init__`, `process_record` and close/drain delegated to `_extraction_pattern`; `key_name` and `storage_client` delegate to pattern; inlined key/handle logic removed; tests updated to patch path modules and assert via `key_name` and mock open.
