...
- **install.sh** — Do not reset PRECOMMIT_FAILED when pre-commit is available; preserve failure state from pip install so final exit status reflects pip/hook failures correctly.
